...
class WorkoutLogFile(WorkoutFile):
    ...
    def parse(self, rename_df_cols: bool = True, *args, **kwargs) -> None:
        ...
        df = pd.concat([df_nmea, df_barometer, df_autopause, df_workout, df_temp, df_location, df_psvlf])
            ...
            if "$" in line:
                ...
                try:
                    ...
                    if msg.identity == "GPGGA":
                        ...
                        data = {
                            "timestamp": timestamp,
                            "navMode_GSA": msg.navMode
                        }
                    elif msg.identity == "GNRMC":
                        data = {
                            "timestamp": timestamp,
                            "speed_RMC": msg.spd * 0.51444,          # Message speed is in knots. 1 kt = 0.5144 m/s
    ...
    def _match_to_timestamp(self, year, timeMatch):
        return year + '-' + timeMatch[0] + '-' + timeMatch[1] + 'T' + timeMatch[2] + ':' + timeMatch[3] + ':' + timeMatch[4] + '.' + timeMatch[5] + 'Z'
